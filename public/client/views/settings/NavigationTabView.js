define([
	...
	'text!templates/settings/NavigationTabView.html',
	...
	var NavigationTabView = Backbone.View.extend({
		...
		submit: function (ev) {
			...
			var self = this,
				...
				href = this.$el.find('input[name="href"]'),
				parent = this.$el.find('select'),
				pullRight = this.$el.find('input[name="pullRight"]').is(':checked'),
				parentDTO = parent.val() ? this.collection.get(parent.val()) : null;

			if (!id.val()) {
				this.collection.create({
					name: name.val(),
					parent: parentDTO,
					parentId: parentDTO ? parentDTO.id : null,
					pullRight: pullRight,
					href: href.val()
				}, {
					success: function () {
				...
				this.collection.get(id.val()).save({
					name: name.val(),
					parent: parentDTO,
					parentId: parentDTO ? parentDTO.id : null,
					pullRight: pullRight,
